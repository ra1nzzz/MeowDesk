...
class SettingsPanel:
    ...
    def _create_general_tab(self):
        """创建常规设置选项卡"""
        tab = self.tk.Frame(self.notebook, bg=self.COLORS['bg'])
        self.notebook.add(tab, text='  常规  ')
        
        row = 0
        
        # 归档目录
        self.tk.Label(tab, text="归档目录", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=(20, 4))
        row += 1
        
        dir_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        ...
        dir_entry = self.tk.Entry(dir_frame, textvariable=self.dir_var, bg=self.COLORS['entry_bg'],
        ...
        # 宠物大小
        self.tk.Label(tab, text="宠物大小", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=(16, 4))
        row += 1
        
        scale_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        ...
        self.scale_label = self.tk.Label(scale_frame, text=self._fmt_scale(self.scale_var.get()),
                                    bg=self.COLORS['bg'], fg=self.COLORS['accent'],
                                    font=("Microsoft YaHei", 10, "bold"), width=6)
        self.scale_label.pack(side='right')
        
        scale_slider = self.tk.Scale(scale_frame, from_=0.3, to=1.0, resolution=0.05,
        ...
        # 随系统启动
        self.tk.Label(tab, text="启动行为", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=(16, 4))
        row += 1
    ...
    def _create_ai_tab(self):
        ...
        self.tk.Checkbutton(tab, text="启用 AI 助手", variable=self.ai_enabled_var,
                      bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                      selectcolor=self.COLORS['bg'], activebackground=self.COLORS['bg'],
                      activeforeground=self.COLORS['fg'],
        ...
        # 网关类型
        self.tk.Label(tab, text="网关类型", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
        ...
        type_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        ...
        # 网关地址
        self.tk.Label(tab, text="网关地址", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10)).grid(
            row=row, column=0, sticky="w", padx=20, pady=(8, 4))
        row += 1
        
        self.endpoint_var = self.tk.StringVar(value=self.config.agent_config.endpoint)
        self.tk.Entry(tab, textvariable=self.endpoint_var, bg=self.COLORS['entry_bg'],
                fg=self.COLORS['fg'], insertbackground=self.COLORS['fg'],
        ...
        # API Token
        self.tk.Label(tab, text="API Token", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10)).grid(
            row=row, column=0, sticky="w", padx=20, pady=(8, 4))
        row += 1
        
        self.token_var = self.tk.StringVar(value=self.config.agent_config.api_key)
        self.tk.Entry(tab, textvariable=self.token_var, bg=self.COLORS['entry_bg'],
                fg=self.COLORS['fg'], insertbackground=self.COLORS['fg'],
        ...
        # 超时时间
        self.tk.Label(tab, text="请求超时（秒）", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10)).grid(
            row=row, column=0, sticky="w", padx=20, pady=(8, 4))
        
        self.timeout_var = self.tk.IntVar(value=self.config.agent_config.timeout)
        self.tk.Spinbox(tab, textvariable=self.timeout_var, from_=5, to=120,
        ...
        # 测试连接按钮
        self.tk.Button(tab, text="测试连接", command=self._test_connection,
                 bg=self.COLORS['accent'], fg="#fff", relief="flat",
    ...
    def _create_reminder_tab(self):
        ...
        # 提醒列表
        list_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        list_frame.pack(fill='both', expand=True, padx=20, pady=(20, 10))
        
        # 标题栏
        header_frame = self.tk.Frame(list_frame, bg=self.COLORS['bg'])
        header_frame.pack(fill='x', pady=(0, 8))
        
        self.tk.Label(header_frame, text="提醒列表", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
        ...
        # 操作按钮
        btn_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
    ...
    def _create_buttons(self):
        """创建底部按钮"""
        self.button_frame = self.tk.Frame(self.window, bg=self.COLORS['bg'])
        self.button_frame.pack(side='bottom', fill='x', padx=20, pady=(0, 16))
        btn_frame = self.button_frame
        
        self.tk.Button(btn_frame, text="保存", command=self._save,
                 bg=self.COLORS['accent'], fg="#fff", relief="flat",
                 cursor="hand2", padx=32, pady=6,
                 font=("Microsoft YaHei", 10, "bold")).pack(side='right', padx=(8, 0))
        
        self.tk.Button(btn_frame, text="取消", command=self.window.destroy,
                 bg=self.COLORS['border'], fg=self.COLORS['fg'], relief="flat",
...
class ReminderDialog:
    ...
    def _create_period_tab(self):
        ...
        self.tk.Checkbutton(tab, text="启用经期提醒", variable=self.period_enabled_var,
                      bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                      selectcolor=self.COLORS['bg'], activebackground=self.COLORS['bg'],
                      activeforeground=self.COLORS['fg'],
                      font=("Microsoft YaHei", 10, "bold")).grid(
        ...
        # 性别/模式选择
        self.tk.Label(tab, text="提醒模式", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=(8, 4))
        row += 1

        self.period_mode_var = self.tk.StringVar(value=period.mode)
        mode_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        ...
        # 周期和经期天数
        cycle_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        cycle_frame.grid(row=row, column=0, columnspan=2, sticky="ew", padx=20, pady=(8, 4))

        self.tk.Label(cycle_frame, text="周期天数", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 font=("Microsoft YaHei", 10)).pack(side='left')
        ...
        self.tk.Label(cycle_frame, text="经期天数", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 font=("Microsoft YaHei", 10)).pack(side='left')
        ...
        # 上次经期日期
        self.tk.Label(tab, text="上次经期日期", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=(12, 4))
        row += 1

        date_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        date_frame.grid(row=row, column=0, columnspan=2, sticky="ew", padx=20, pady=(0, 4))

        self.tk.Label(date_frame, text="首日:", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 font=("Microsoft YaHei", 10)).pack(side='left')
        self.last_start_var = self.tk.StringVar(value=period.last_period_start or "")
        self.tk.Entry(date_frame, textvariable=self.last_start_var, bg=self.COLORS['entry_bg'],
                fg=self.COLORS['fg'], insertbackground=self.COLORS['fg'],
                relief="flat", font=("Microsoft YaHei", 10), width=12).pack(side='left', padx=(4, 16))

        self.tk.Label(date_frame, text="结束日:", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 font=("Microsoft YaHei", 10)).pack(side='left')
        self.last_end_var = self.tk.StringVar(value=period.last_period_end or "")
        self.tk.Entry(date_frame, textvariable=self.last_end_var, bg=self.COLORS['entry_bg'],
                fg=self.COLORS['fg'], insertbackground=self.COLORS['fg'],
                relief="flat", font=("Microsoft YaHei", 10), width=12).pack(side='left', padx=(4, 0))

        self.tk.Label(date_frame, text="(YYYY-MM-DD)", bg=self.COLORS['bg'], fg='#6b7280',
        ...
        self.pred_label = self.tk.Label(tab, text=pred_text, bg=self.COLORS['bg'], fg=self.COLORS['accent'],
                                  font=("Microsoft YaHei", 10), wraplength=450)
        self.pred_label.grid(row=row, column=0, columnspan=2, padx=20, pady=(8, 4))
        row += 1

        # 校准区域
        self.tk.Label(tab, text="校准调整", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=(12, 4))
        row += 1

        calib_frame = self.tk.Frame(tab, bg=self.COLORS['bg'])
        calib_frame.grid(row=row, column=0, columnspan=2, sticky="ew", padx=20, pady=(0, 8))

        self.tk.Label(calib_frame, text="偏移天数:", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                 font=("Microsoft YaHei", 10)).pack(side='left')
        ...
        # 历史记录
        if period.records:
            self.tk.Label(tab, text="历史记录 (最近3次)", bg=self.COLORS['bg'], fg=self.COLORS['fg'],
                     anchor="w", font=("Microsoft YaHei", 10, "bold")).grid(
                row=row, column=0, sticky="w", padx=20, pady=(8, 4))
            row += 1
            ...
            for i, record in enumerate(period.records[-3:]):
                ...
                self.tk.Label(tab, text=record_text, bg=self.COLORS['bg'], fg='#9ca3af',
